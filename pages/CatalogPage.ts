...
export class CatalogPage extends LearnerHomePage {
    public selectors = {
        ...
        searchButton: `(//span[text()='Tags']/following::input[@placeholder='Select'])[1]`,
    ...
    async playAssessmentVideo() {//jagadish
        await this.page.evaluate(() => {
            return Promise.all(Array.from(document.querySelectorAll('video')).map(video => {
    ...
    async bookmarkClass(clsName: string) {
        await this.spinnerDisappear();
        await this.wait("mediumWait");
        await this.validateElementVisibility(this.selectors.classBookmark, "Class Bookmark Icon")
        await this.click(this.selectors.classBookmark(clsName), "Bookmarked the Class", "Icon");
    ...
    async learnWithin30Mins() {
        await this.validateElementVisibility(this.selectors.learnWithinMins, "Learn within 30 mins");
        await this.mouseHover(this.selectors.learnWithinMins, "Learn within 30 mins");
    }

    //To launch the already completed course from watchItAgain section
    async watchItAgain() {
        await this.validateElementVisibility(this.selectors.watchItAgain, "Learn within 30 mins");
    ...
    async launchContentFromMylearning() {
        await this.wait("mediumWait")
        await this.validateElementVisibility(this.selectors.contentPlayBtn, "Content Play");
    ...
    async verifyContentProgressValue(contentName: string) {
        ...
        await this.verification(this.selectors.contentProgressValue(contentName), "100%")
    }
    ...
    public async verifytpCourseStatus(data: string, status: string) {
        await this.verification(this.selectors.tpCourseStatus(data, status), status)
    }


    public async clickEqlConfirmationPopup(option: string) {
        await this.validateElementVisibility(this.selectors.confimationPopupEql(option), "Equivalence Confirmation PopUp");
        await this.click(this.selectors.confimationPopupEql(option), "Equivalence Confirmation PopUp", "Button")
        await this.wait(`maxWait`);
    }
    public async verifyEquivalenceGrantedMessage() {
        await this.verification(this.selectors.grantedMessageForEql, "YOU HAVE BEEN GRANTED COMPLETION FOR THIS COURSE");
    }
    public async verifyPrerequisiteMandatoryMessage(data?: string) {
        if (data == "Training Plan") {
            await this.verification(this.selectors.prerequisiteMandatoryMessage, "prerequisite training needs to be completed.");
            await this.click(this.selectors.okBtn, "Prerequisite Mandatory Message", "Button")
        } else {
            await this.verification(this.selectors.prerequisiteMandatoryMessage, "Prerequisite course needs to be completed.");
            await this.click(this.selectors.okBtn, "Prerequisite Mandatory Message", "Button")
        }
    }
    ...
    public async clickCourseOnDetailsPage(courseName: string) {
        await this.validateElementVisibility(this.selectors.clickCourseDetailsPage(courseName), "Link");
        await this.click(this.selectors.clickCourseDetailsPage(courseName), "My Learning", "Link");
    ...
    async changeClass() {
        await this.wait("mediumWait");
        await this.click(this.selectors.mylearningChangeClass, "Change Class Option", "Link");

    }
    //Change class confirmation:-
    async verifyChangeClass() {
        let result = await this.page.locator(this.selectors.okBtn).nth(0).innerText();
        expect(result).toContain("Success")
        await this.click(this.selectors.okBtn, "Confirm Popup", "Button")
    }

    async playVimeo() {  //This method will allow the video to play until it reaches 0 seconds remaining.
    ...
    async mylearningViewClassDetails(clsTitle: string) {
        await this.click(this.selectors.mylearningViewClassDetails(clsTitle), "MyLearning Class Title", "Link")
    }

    //Mandatory text verification:-
    async mandatoryTextVerification() {
        let result = await this.page.locator(this.selectors.mandatoryText).nth(0).innerText();
        expect(result).toContain('MANDATORY')

    }

    async tpCourseSearch(data: string) {
        await this.click(this.selectors.clickTPCourseFilter, "Filter", "TP Course Filter Link");
        await this.click(this.selectors.clickTPCourseDeliveryFilter(data), "Filter", "Checkbox");
        await this.click(this.selectors.clickTPCourseApplyButton, "TP Course Filter Apply", "Button");
        await this.wait("minWait");

    }

    async mylearningNoResultsFound() {
        await this.validateElementVisibility(this.selectors.noResultFound, "No Result Found");
        let result = await this.page.locator(this.selectors.noResultFound).nth(0).innerText();
        expect(result).toContain('No results found')
    }

    async mylearningClassCancel() {
        await this.wait("minWait");
        await this.click(this.selectors.mylearningClassCancel, "Cancel Enrollment", "Button")
        await this.type(this.selectors.mylearningcancelreason, "Cancel Reason", "Simply cancelling the record")
        await this.click(this.selectors.mylearningyesbutton, "Yes button", "Button")
        await this.wait("maxWait");
        let result = await this.page.locator(this.selectors.mylearningcancelverification).nth(0).innerText();
    ...
    async mylearningMandatoryClassCancel() {
        await this.wait("minWait");
        await this.click(this.selectors.mylearningClassCancel, "Cancel Enrollment", "Button");
        let result = await this.page.locator(this.selectors.mandatoryAlertMessage).nth(0).innerText();
        expect(result).toContain('This training is Mandatory and cannot be canceled')
    }

    async tpCourseSelection() {
        await this.wait('minWait')
        await this.click(this.selectors.selectTPCourse, "TP Course Selection", "Radio Button");
    }

    async tpSelectedCourseRegister() {
        await this.click(this.selectors.clickTPSelectedCourseRegisterButton, "TP Selected Course Selection", "Button");
        await this.wait("mediumWait")
    }
    ...
    async noResultFound() {
        ...
        await this.validateElementVisibility(this.selectors.noResultFound_MostRecent, "No Result Found");
        let result = await this.getInnerText(this.selectors.noResultFound_MostRecent);
        expect(result).toContain('No results found')
        await this.wait("mediumWait");
    }
    ...
    async verifyOverdue(data: string) {
        await this.wait('maxWait');
        await this.mouseHover(this.selectors.completedCourse(data), "Overdue Course/TP");
    ...
    async vertifyTPIncomplete(data?: string) {
        ...
        await this.validateElementVisibility(this.selectors.incompleteText, "Incomplete");
    ...
    async verifyCourseIncomplete(data: string) {
        await this.wait('minWait');
        await this.validateElementVisibility(this.selectors.courseIncompleteText, "In complete");
        await this.verification(this.selectors.courseIncompleteText, "In complete")
    }

    async clickMoreonCourse(courseName: string) {
        await this.wait("minWait");
        await this.mouseHover(this.selectors.clickCourse(courseName), "More on Course")
        await this.click(this.selectors.clickCourse(courseName), "More on Course", "icon")
    }
    ...
    async clickRecertify() {
        ...
        await this.page.locator(this.selectors.completedVideo).scrollIntoViewIfNeeded({ timeout: 5000 });
        await this.wait('mediumWait');
    }
    ...
    async verifyEnrolledCourseByCODE(code: string) {
        await this.wait('minWait');
        await this.click(this.selectors.toCompleteORCompleteEnrolledCourse, code, "Link");
        await this.spinnerDisappear()
    ...
    async clickSubmitSurvey() {
        ...
        if (await submitSurvey.isVisible()) {
            ...
            await this.click(this.selectors.sumbitSurveyBtn, "Submit My Answer", "Button");
            await this.wait(`mediumWait`);
            await this.spinnerDisappear();
        }
    }
    async surveyPlayButton() {
        ...
        await this.validateElementVisibility(this.selectors.surveyPlayBtn, "Survey Button")
    ...
    async submitMyAnswer() {
        ...
        if (await submitBtn.isVisible()) {
            ...
            await this.wait(`minWait`);
            let negativeAnsDoneButton = this.page.locator(this.selectors.negAnsDoneButton)
            await this.wait(`minWait`);
    ...
    async surveySumbitBtn() {
        ...
        if (await surveySubmitBtn.isVisible()) {
            ...
            await this.wait(`minWait`);
            await this.click(this.selectors.surveyDoneButton, "TP Survey Done Button", "Button")
            await this.spinnerDisappear();
        }
    }
    async clickReenroll() {
        let verification = this.page.locator(this.selectors.incompleteText).innerHTML();
        expect("Incomplete").toContain(await verification)
        await this.validateElementVisibility(this.selectors.reenrollbutton, "re-enroll button is visible");
        await this.click(this.selectors.reenrollbutton, "TP re-enroll", "Button")
    }

    async tpCompleteionVerification() {
        let verification = this.page.locator(this.selectors.tpCompletedText).innerHTML();
        expect("Completed").toContain(await verification)

    }

    async tpPreAssessmentLaunch() {
        await this.validateElementVisibility(this.selectors.tpPreAssbutton, "tpPreassessmentButton");
        await this.click(this.selectors.tpPreAssbutton, "tpPreassessmentButton", "Button")
        await this.spinnerDisappear();
    }


    async tpPostAssessmentLaunch() {
        await this.validateElementVisibility(this.selectors.tpPostAssbutton, "tpPostassessmentButton");
        await this.click(this.selectors.tpPostAssbutton, "tpPostassessmentButton", "Button")
        await this.spinnerDisappear();
    }

    //No result found in mylearning->To Complete
    async noResultFoundOnMyLearning() {
        await this.validateElementVisibility(this.selectors.noResultFound_Mylearning, "No Result Found");
        let result = await this.getInnerText(this.selectors.noResultFound_Mylearning);
        expect(result).toContain('No results found')
    }
    async verifyThumbnailImage(data: string) {
        const lnrSrcValue = await this.page.locator(this.selectors.thumbnailImgSrc).getAttribute('src')
        console.log(lnrSrcValue)
        expect(lnrSrcValue).toContain(data);
    }

    //for playing multi conent playing based on content title
    async contentPlayByName(data: string) {
        await this.wait("mediumWait")
        await this.validateElementVisibility(this.selectors.contentPlay(data), "Content Play");
        await this.click(this.selectors.contentPlay(data), "Content Play", "Button");
    }
    ...
    async verifyFailedMessage() {
        ...
        await this.verification(this.selectors.ContentExpireCheck, " can no longer be launched as the validity has expired or there are no attempts left.",);
    }

    //Verifying no seats left message on leaner side
    async verifySeatStatus() {
        await this.wait("mediumWait")
        await this.validateElementVisibility(this.selectors.noSeatLeftPopupMsg, "No seat left");
    ...
    async clickOnNextCourse(data: string) {
        ...
        await this.click(this.selectors.clickNextCourse(data), "Select Next course", "Link")
        await this.wait("maxWait")
    }

    async clickSessionConflictPopup() {
        await this.wait("minWait")
        const sessionConflict = this.page.locator(this.selectors.sessionConflictPopup);
        if (await sessionConflict.isVisible()) {
            await this.click(this.selectors.mylearningyesbutton, "Session Conflict Popup", "Button")
            await this.wait("maxWait")
        } else {
    ...
    async dclmesageVerification(){
        await this.validateElementVisibility(this.selectors.DCLNotEnrolledMessage, "DCL message");
        await this.verification(this.selectors.DCLNotEnrolledMessage, "You are not enrolled in the class");
