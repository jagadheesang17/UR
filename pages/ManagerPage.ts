...
export class ManagerPage extends PlaywrightWrapper {
    ...
    public selectors = {
        ...
        guideTeam: (courseName: string) => `(//div[text()='${courseName}']/following::i[@aria-label='Guide Team'])[1]`,
    ...
    async enterSearchCourse(data: string) {
        ...
        await this.validateElementVisibility(this.selectors.searchCourse, "Search Text Field")
    ...
    async clickGuideTeamIcon(data:string){
        await this.wait('mediumWait')
        await this.validateElementVisibility(this.selectors.guideTeam(data),"Guide Team")
        await this.mouseHover(this.selectors.guideTeam(data),"Guide Team")
        await this.page.locator(this.selectors.guideTeam(data)).click({ force: true });
    ...
    async clickTeamEnrollementStatus(){
        await this.wait('mediumWait')
        await this.validateElementVisibility(this.selectors.viewUpdateStatus,"Team Enrollment Status")
        await this.click(this.selectors.viewUpdateStatus, `Team Enrollment Status`, "Link")
        await this.page.waitForLoadState('load');
        await this.wait('maxWait')
    }
    public async searchTeamUser(data: string) {
        await this.wait('minWait')
        await this.typeAndEnter(this.selectors.searchTeamUser, "Search Team User", data);
        await this.wait('minWait')
