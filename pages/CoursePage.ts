...
import { FakerData, getallRandomInstructor, getallRandomLocation, getCurrentDateFormatted, getFutureDate, getnextMonthFormatted, getPastDate, getRandomFutureDate, getRandomLocation, getRandomSeat, gettomorrowDateFormatted, score } from "../utils/fakerUtils";
...
export class CoursePage extends AdminHomePage {
    ...
    // Alias to support camelCase usage in tests
    async enterCode(code: string) {
    ...
    async noResults() {
        await this.validateElementVisibility(this.selectors.noResults, "No Results Found");
    }
    ...
    async editInstanceFromCrsListing() {
        await this.validateElementVisibility(this.selectors.instanceIcon, "Instances");
        await this.click(this.selectors.instanceIcon, "Instances", "Icon");
        await this.click(this.selectors.editInstanceFromCrsList, "Instances", "Icon");
        await this.wait("minWait");
    }

    //edit session details
    async clickEditSession() {
        await this.validateElementVisibility(this.selectors.editSession, "Edit Session");
        await this.click(this.selectors.editSession, "Edit Session", "Icon");
        await this.wait("minWait");
    }
    //update session
    async clickUpdateSession() {
        await this.validateElementVisibility(this.selectors.updateSession, "Edit Session");
        await this.click(this.selectors.updateSession, "Edit Session", "Icon");
    }

    async enterSessionName_bulk(sessionName: string, i: any) {
        await this.validateElementVisibility(this.selectors.sessionNameInput_bulk(i), "Session Name");
        await this.mouseHover(this.selectors.sessionNameInput_bulk(i), "Session Name")
        await this.type(this.selectors.sessionNameInput_bulk(i), "Session Name", sessionName);
    }
    async enterSessionName_copy(sessionName: string,) {
        await this.validateElementVisibility(this.selectors.sessionNameInput_Copy, "Session Name");
        await this.mouseHover(this.selectors.sessionNameInput_Copy, "Session Name")
        await this.type(this.selectors.sessionNameInput_Copy, "Session Name", sessionName);
    }
    async selectInstructor_bulk(instructorName: string, i: any) {
        await this.click(this.selectors.instructorDropdown_bulk(i), "Select Instructor", "DropDown");
        await this.type(this.selectors.instructorInput_bulk(i), "Instructor Name", instructorName);
    ...
    async selectInstructor_Copy(instructorName: string) {
        await this.click(this.selectors.instructorDropdown_Copy, "Select Instructor", "DropDown");
        await this.type(this.selectors.instructorInput_Copy, "Instructor Name", instructorName);
        await this.mouseHover(this.selectors.instructorOption_Copy(instructorName), "Instructor Name");
        await this.click(this.selectors.instructorOption_Copy(instructorName), "Instructor Name", "Button")
    }
    async selectLocation_bulk(i: any) {
        await this.click(this.selectors.locationSelection_bulk(i), "Select Location", "DropDown");
        await this.click(this.selectors.locationDropdown_bulk(i), "Select Location", "DropDown");
        let location = getallRandomLocation();
        await this.type(this.selectors.locationDropdown_bulk(i), "Select Location", location);
        await this.mouseHover(this.selectors.locationOption_bulk(location, i), "Location");
        await this.click(this.selectors.locationOption_bulk(location, i), "Location", getallRandomLocation());
    }
    async selectLocation_Copy() {
        await this.click(this.selectors.locationSelection_Copy, "Select Location", "DropDown");
        await this.click(this.selectors.locationDropdown_Copy, "Select Location", "DropDown");
        let location = getRandomLocation()
        await this.type(this.selectors.locationDropdown_Copy, "Select Location", location);
        await this.mouseHover(this.selectors.locationOption_Copy(location), "Location");
        await this.click(this.selectors.locationOption_Copy(location), "Location", getRandomLocation());
    }
    ...
    async captureDropdownValues(i: any, str: string,): Promise<void> {
        ...
        const dropdownValues: string[] = await this.page.locator("//li[contains(@id,'list')]").allInnerTexts();
...
async captureDropdownValuesOfLocationCopy(str: string,): Promise<void> {
    ...
    async verifyCreatedBulkClasses(Nos: any) {
        for (let k = 1; k <= Nos; k++) {
            ...
            await this.page.locator(this.selectors.clickEditBulkClass(k)).scrollIntoViewIfNeeded();
            await this.validateElementVisibility(this.selectors.clickEditBulkClass(k), "edit");
            await this.wait("maxWait");
            ...
            await this.page.locator(this.selectors.instance_Class).click({ force: true })
            await this.wait("mediumWait")
            const addInstanceBtn = await this.page.locator(this.selectors.addInstancesBtn).isVisible()
            if (!addInstanceBtn) {
                await this.wait("minWait")
                await this.clickinstanceClass();
    ...
    public async startandEndTime_bulkClass(i: any) {
        await this.click(this.selectors.timeInput_bulk(i), "Start Time Input", "Input");
        await this.wait('minWait');
        ...
        async function selectNextAvailableTime() {
            ...
            await this.page.locator(`(//div[contains(@class,'timepicker')]//li[text()='${timeToSelect}'])[${i + 1}]`).click();
    ...
    async setMaxSeat_bulk(i: any) {
        await this.typeAndEnter(this.selectors.seatMaxInput_bulk(i), "Instance Max Seat", await getRandomSeat());
    }
    async setMaxSeat_Copy() {
        await this.typeAndEnter(this.selectors.seatMaxInput_Copy, "Instance Max Seat", await getRandomSeat());
    }
    //Class enrollment E-Learn as an admin from the course edit page:-
    ...
    async filterByStatus(data: string) {
        await this.wait("minWait")
        await this.click(this.selectors.filterInCourseList, "Filter by Instance", "Button");
        await this.validateElementVisibility(this.selectors.searchType, "Search Type")
        await this.click(this.selectors.statusDropdown, "Status Dropdown", "Button");
        await this.click(this.selectors.selectStatus(data), "Search Type Option", "Option");
    }
    ...
    async clickClassEnrollmentILTVCType() {
        await this.wait("minWait");
        await this.validateElementVisibility(this.selectors.classEnrollBtn, "Enrollment");
    ...
    async selectDuration(Duration: string) {
        await this.typeAndEnter(this.selectors.selectDuration, "Duration", Duration)
    }

    //course Dedicated to TP:-
    async clickCourseDedicatedToTPRule() {
        await this.wait("mediumWait")
        await this.click(this.selectors.courseBusinessRulesLink, "Course Business Rule Link", "Link");
        await this.validateElementVisibility(this.selectors.courseDedicatetoTP, "Course Dedicated to TP");
        await this.click(this.selectors.courseDedicatetoTP, "Course Dedicated to TP", "Checkbox");
        await this.click(this.selectors.okButton, "Ok button present in the popup", "Ok Button");
        await this.click(this.selectors.saveEquivalenceButton, "Save Button", "Button");
    }

    //To set Prerequisite/Equilance for course:-
    async clickCourseOption(option: string) {
        await this.wait("mediumWait")
        await this.validateElementVisibility(this.selectors.courseOption(option), "Course Option");
        await this.click(this.selectors.courseOption(option), "Course Option", "Button");
    }

    //get conetent name on course creation page:-
    async getAttachedContentName() {
        let contentName = await this.getInnerText(this.selectors.contentTitleOnCoursePage)
        return contentName;
        console.log("Content name is: " + contentName);
    }

    async addMultiPrerequisiteCourse(courseName: string, courseName1: string) {
        await this.typeAndEnter(this.selectors.preSearchField, "Completion Certificate Search Field", courseName);
        await this.spinnerDisappear();
        await this.click(this.selectors.preCourseIndex(1), "Prerequisite Course", "checkbox");
        await this.wait('minWait');
        ...
        await this.wait('mediumWait');
        await this.typeAndEnter(this.selectors.preSearchField, "Completion Certificate Search Field", courseName1);
        await this.spinnerDisappear();
        await this.click(this.selectors.preCourseIndex(1), "Prerequisite Course", "checkbox");
        await this.wait('minWait');
        await this.mouseHover(this.selectors.addPreCourseBtn, "addcontent");
    ...
    async clickSpecificCertificateCheckBox(data: string) {
        await this.typeAndEnter('#exp-search-certificate-field', "Completion Certificate Search Field", data);
        await this.spinnerDisappear();
        await this.mouseHover(this.selectors.clickCreatedCertificateCheckbox(data), "Created Certificate CheckBox");
        await this.click(this.selectors.clickCreatedCertificateCheckbox(data), "Created Certificate CheckBox", "Checkbox");
    }

    async addEquivalenceCourse(coursename: string) {
        await this.wait("minWait")
        await this.validateElementVisibility(this.selectors.selectEquivalenceCourse(coursename), "Equivalence Course");
        await this.click(this.selectors.selectEquivalenceCourse(coursename), "Equivalence Course", "Radio Button");
        await this.click(this.selectors.addEquivalenceButton, "Add Equivalence Course", "Button");
        await this.wait("minWait")
        await this.click(this.selectors.saveEquivalenceButton, "Save Equivalence Course", "Button");
        await this.wait("minWait")
        await this.verification(this.selectors.equivalenceSuccessMessage, "successfully");
    }
    ...
    async catalogSearch(data: string) {
        ...
        await this.keyboardAction(this.selectors.crs_TPSearchField, "Enter", "Search Field", data);
        await this.spinnerDisappear();
    }
    async retriveCode() {
...
await this.page.locator("label[for='publishedcatalog'] span", { hasText: 'Show in Catalog' }).click();
  ...
  console.log("Proceed button not visible even after 6 attempts. Save may have failed.");
    ...
    async selectDomainOption(portalName: string) {
        ...
        for (const options of await this.page.locator(this.selectors.allPortalOptions).all()) {
            ...
            try {
                ...
                if (value !== portalName) {
                    try {
                        await this.page.click(`//div[@id='wrapper-course-portals']//span[@class='text' and text()='${value}']`, { timeout: 2000 });
                    } catch (error) {
                        // console.error("Error clicking on the first locator, trying the fallback locator:", error);
                        await this.page.click(`//footer//following::span[@class='text' and text()='${value}']`, { timeout: 2000 });
                    }
                }
            } catch (error) {
    ...
    async enterPrice(price: string) {
        await this.page.locator(this.selectors.priceInput).scrollIntoViewIfNeeded({ timeout: 3000 })
        await this.type(this.selectors.priceInput, "Price", price);
    }

    async selectCurrency() {
        await this.page.locator(this.selectors.currencyDropdown).scrollIntoViewIfNeeded({ timeout: 3000 })
    ...
    async selectInstanceType(instanceType: string) {
        await this.click(this.selectors.selectInstanceDropdown, "Instance Dropdown", "dropdown");
        await this.wait("minWait");
        await this.mouseHover(this.selectors.instanceSelection(instanceType), "Multiinstance");
        await this.click(this.selectors.instanceSelection(instanceType), "Multiinstance", "Selected");
    }
    ...
    async selectCompleteByDate() {
        ...
        await this.keyboardType(this.selectors.CourseCalendaricon, gettomorrowDateFormatted())
        //await this.wait("minWait")
    ...
    public async startandEndTime() {
        ...
        async function selectNextAvailableTime() {
            ...
            const timeToSelect = getCurrentTimePlusTwoHours();
            console.log('Current Time + 2 hours:', timeToSelect);
            await this.page.locator(`(//div[contains(@class,'timepicker')]//li[text()='${timeToSelect}'])`).click();
    ...
    async addSpecificAssesment(data: string) {
        await this.mouseHover(this.selectors.assessmentLabel, "Assessment");
        await this.typeAndEnter(this.selectors.searchAssessmentField, "Assessment Search", data);
        await this.spinnerDisappear();
        await this.click(this.selectors.assessmentCheckbox, "Assessment", "CheckBox");
        await this.click(this.selectors.addAssessmentBtn, "Addassesment", "button");
        await this.wait('maxWait')
    }

    async setAssessmentAttempt() {
        let count = await this.page.locator("//span[contains(@id,'attemptssapn')]").count();
        for (let i = 0; i < count; i++) {
            await this.page.locator("//span[contains(@id,'attemptssapn')]").nth(i).click({ clickCount: 2, delay: 1000 });
            await this.page.locator("//span[contains(@id,'attemptssapn')]").nth(i).focus();
    ...
    async handleCategoryADropdown(): Promise<string> {
        ...
        await this.click(this.selectors.categoryOption(categoryName), "options", "button");
        return categoryName;
    }
    ...
    async providerDropdown() {
        ...
        const randomIndex = Math.floor(Math.random() * (await providerElements.count() - 1)) + 1;
        // await this.click(this.selectors.providerDropdown, "dropdown", "button")
    ...
    async addSpecificSurveyCourse(data: string) {
        ...
        await this.click(this.selectors.surveyAndAssessmentLink, "Survey/Assessment", "Link")
        await this.wait('mediumWait')
        const popup = this.page.locator("(//span[text()='You have unsaved changes that will be lost if you wish to continue. Are you sure you want to continue?'])[1]")
        if (await popup.isVisible({ timeout: 5000 })) {
            await this.click("//button[text()='YES']", "yes", "button")
        }
        await this.typeAndEnter(this.selectors.surveySearchField, "Survey Search Field", data);
        await this.spinnerDisappear();
    ...
    async specificLearnerGroupSelection(learnerGroupName: string) {
        ...
        for (const options of await this.page.locator(this.selectors.allLearnerGroupOptions).all()) {
            ...
            console.log(value)
            if (value !== learnerGroupName) {
                const labelLocator = this.page.locator(`//footer//following::span[@class='text' and text()='${value}']`);
                const checkMarkLocator = labelLocator.locator('..').locator("span.check-mark");
    ...
    async crsAccessSettings() {
        await this.wait("minWait")
        await this.click(this.selectors.crsAccessSettingLink, "Access Setting Link", "Link");
        await this.click(this.selectors.crsAccessDropDown, "Access Dropdown ", "Dropdown");
        await this.click(this.selectors.crsAccessMandatoryOption, "Mandatory Selection", "Dropdown");
        await this.wait("minWait")
        await this.click(this.selectors.crsAccessUserDropDown, "User Access Dropdown ", "Dropdown");
        await this.click(this.selectors.crsAccessUserMandatoryOption, "Mandatory Selection", "Dropdown");
        await this.click(this.selectors.crsAccessSettingsSave, "Mandatory Selection", "Dropdown");
    ...
    async verifyCurrencyNotPresent(currencyName: string): Promise<void> {
        ...
        for (const options of await this.page.locator(this.selectors.currencyListInCourse).all()) {
            const value = await options.innerText();
            console.log(value)
    ...
    async filterByInstance(data: string) {
        ...
        await this.click(this.selectors.filterInCourseList, "Filter by Instance", "Button");
        await this.validateElementVisibility(this.selectors.searchType, "Search Type")
        await this.click(this.selectors.searchTypeDropdown, "Search Type Dropdown", "Button");
        await this.click(this.selectors.deliveryTypeOption(data), "Search Type Option", "Option");
    }
    ...
    async addThumbnailImagefromSystemGallery() {
        ...
        await this.wait("minWait")
        const srcValue = await this.page.locator(this.selectors.uploadedImgSrc).getAttribute('src')
    ...
    async uploadThumbnailfromCustomGallery() {
        await this.wait("minWait")
        await this.click(this.selectors.clickHere, "Click Here", "Link");
        await this.validateElementVisibility(this.selectors.customGalleryRadioBtn, "Custom Gallery")
        await this.click(this.selectors.customGalleryRadioBtn, "Custom Gallery", "Radio Button");
        ...
        await this.wait("mediumWait")
        await this.page.locator(this.selectors.addThumbnail).setInputFiles(randomFilePath);
        await this.wait("mediumWait")
        const srcValue = await this.page.locator(this.selectors.uploadedImgSrc).getAttribute('src')
        console.log(srcValue)
        return srcValue;
    ...
    async contentValidity(validity: string, data: string) {
        ...
        await this.click(this.selectors.selectContentValidity, "validity", "content")
        await this.mouseHover(this.selectors.validityType(validity), "ValidityType")
        await this.click(this.selectors.validityType(validity), "validityType", "content")
        if (validity === "Date") {
    ...
    async setSeatsMax(seatCount: any) {
        await this.typeAndEnter(this.selectors.seatMaxInput, "Instance Max Seat", seatCount);
    }

    //Recurring class creartion
    async selectSessionType() {
        await this.wait("minWait")
        await this.click(this.selectors.selectSessionType, "Session Type", "Button");
    }
    async selectAllDays() {
        let value: any
        ...
        // }
        for (let i = 0; i < count; i++) {
            const option = await this.page.locator(this.selectors.selectDays(i + 1)).innerText();
            await this.click(this.selectors.daysOption(option), "Select days ", "Option")
        }
    }
    ...
    async checkContactSupportNew(expectedEmail: string) {
        await this.validateElementVisibility(this.selectors.checkContactSupport, "Contact");
        // await this.verificationInputValue(this.selectors.checkContactSupport."")
        await this.verificationInputValue(this.selectors.checkContactSupport, expectedEmail)
    }
