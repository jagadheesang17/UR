...
/** Keep in sync with `environmentSetup` in playwright.config.ts */
type MetadataLibraryEnv =
    | "qa"
    | "dev"
    | "automation"
    | "qaProduction"
...
const pathsByEnvironment: Record<MetadataLibraryEnv, typeof productionPaths> = {
    qa: qaPaths,
    dev: devPaths,
    automation: automationPaths,
    qaProduction: productionPaths,
    urAutoTenant: productionPaths,
};

const resolved =
    pathsByEnvironment[environmentSetup as MetadataLibraryEnv] ?? null;
if (resolved === null) {
    throw new Error(`Invalid environment setup: ${environmentSetup}`);
}

export let filePath: any = resolved;
