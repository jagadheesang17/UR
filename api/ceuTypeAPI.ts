...
export async function createCEUType(code: string, authorization: any) {
    let response = await postRequest(ceuTypeCreationData(code), endPointURL, authorization);
...
export async function ceu_type_list(id: any, authorization: any,) {
    let response = await postRequest(ceuGetListOfData, endPointURL, authorization);
...
export async function delete_ceu_type(code: string, authorization: any) {
    let response = await postRequest(ceuTypeDelete(code), endPointURL, authorization);
    console.log(response.data);
