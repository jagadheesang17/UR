...
import { getListAttachedCoursesFromPrograms } from "../../programsAPI";
...
test(`Verify_Learning_Path__single_instance_with_attached_created_course`, async ({ contentHome, adminHome, learningPath, createCourse }) => {
    test.info().annotations.push(
        { type: `Author`, description: `Tamilvanan` },
...
test('Fetch List of Learning Paths', async () => {
    ...
    await getListAttachedCoursesFromPrograms(program_code, { Authorization: access_token });
});
