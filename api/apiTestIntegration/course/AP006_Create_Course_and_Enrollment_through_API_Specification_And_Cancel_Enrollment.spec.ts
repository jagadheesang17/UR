...
import { CancelEnrolledCourse, completeEnrolledCourse, enrollCourse, listEnrolledCourse } from "../../courseAPI";
...
test.describe(`Creating a course in the UI, enrolling through the API, and finally verifying on the learner's side in the UI`, async () => {
    ...
    test(`Complete Enrolled Course through API`, async () => {
        await CancelEnrolledCourse(createdCode, user, { Authorization: access_token })
    })

    test(`Verify Completed Course in Learner side`, async ({ learnerHome, catalog }) => {
        await learnerHome.learnerLogin("LEARNERUSERNAME", "DefaultPortal");
        await catalog.clickCatalog();
        await catalog.searchCatalog(createdCode);
    })

    test(`List the cancelled enrolled course`, async () => {
