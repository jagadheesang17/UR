...
const courseCode = `EA009-${codeSuffix}`;
const elCourseCode = `EA9EL-${codeSuffix}`;
...
test.describe(`TC103 Verify future Class recurring Registration`, async () => {
    ...
    test(`TC103_Multiple Course Recurring registration for Future date`, async ({ createCourse, adminHome, editCourse, contentHome, catalog, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Vidya` },
        ...
        await createCourse.entercode(elCourseCode);
    ...
    test(`Verification from learner site`, async ({ learnerHome, learnerCourse, catalog,dashboard }) => {
        test.info().annotations.push(
            { type: `Author`, description: `vidya` },
       ...
       await dashboard.selectDashboardItems("Learning History");
        await dashboard.learningHistoryCourseSearch(elCourseName);
