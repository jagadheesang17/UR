...
const courseCode = `EA17C-${codeSuffix}`;
const certificationCode = `EA17T-${codeSuffix}`;
...
test.describe(`EA017_Certification_Recertification_Enroll_Again_Restriction`, async () => {
    ...
    test(`Create_Certification_Verify_Recert_Popup_Then_Enable_Enroll_Again`, async ({ adminHome, learningPath, createCourse, editCourse,enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
        ...
        await learningPath.enterCode(certificationCode);
    ...
    test(`Verification_From_Learner_Site`, async ({ dashboard,learnerHome, catalog }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
        ...
        await catalog.clickMoreonCourse(certificationName);
