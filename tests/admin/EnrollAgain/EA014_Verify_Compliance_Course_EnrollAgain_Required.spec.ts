...
const courseCode = `EA014-${`${Date.now()}`.slice(-5)}`;
...
test.describe(`TC108 Verify Compliance Course Requires Enroll Again Enabled`, async () => {
    ...
    test(`TC108_Create_Compliance_Course_Without_EnrollAgain_And_Enable_It`, async ({ adminHome, learningPath, createCourse, editCourse, contentHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
