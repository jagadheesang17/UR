...
const courseCode = `EA008-${`${Date.now()}`.slice(-5)}`;
...
test.describe(`TC102 Course creation for E-learning recurring registration`, async () => {
    ...
    test(`TC102_CreateCourseForElearning_Recurring_Registration`, async ({ adminHome, createCourse, editCourse, contentHome, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Vidya` },
