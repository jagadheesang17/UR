...
const courseCode = `EA001-${`${Date.now()}`.slice(-5)}`;
...
test.describe(`EA001 Verify Enroll Again Functionality`, async () => {
    ...
    test(`EA001_Create_Course_And_Verify_Allow_Learners_To_Enroll_Again_Checkbox`, async ({ adminHome, createCourse, editCourse, enrollHome }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
    ...
    test(`EA001_Learner_Side_Verification_Enroll_Again`, async ({ learnerHome, learnerCourse, catalog, dashboard }) => {
        test.info().annotations.push(
            { type: `Author`, description: `Tamilvanan` },
        ...
        await catalog.saveLearningStatus();
        await catalog.clickMyLearning();
        // Step 7: Verify the course appears in Learning History (same pattern as BOOK001, ENR_VUS_003, etc.)
