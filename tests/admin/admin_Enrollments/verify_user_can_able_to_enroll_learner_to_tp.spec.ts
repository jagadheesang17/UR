...
// test.describe.serial(`Confirm that Admin enrollments functions correctly and as expected for ILT course`, () => {
//     test.describe.configure({ mode: "serial" });
//     test(`Creation of ILT Course`, async ({ adminHome, createCourse, enrollHome, contentHome }) => {
...
test.describe(`Certification_with_multi_instance_course_is_attached`, () => {
    test.describe.configure({ mode: "serial" });
    ...
    test(`Multi-Instance creation Classroom and Elearning`, async ({ adminHome, createCourse, editCourse }) => {
        ...
        await editCourse.clickClose();
        /* Previously blocked by Automation Site issue (20-6-2024) — re-enabled */
    ...
    test(`Creation of certification with multi instance course is attached`, async ({ enrollHome, adminHome, learningPath, createCourse }) => {
        ...
        await createCourse.catalogSearch("Primary Transmitter Index");
