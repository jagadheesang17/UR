  ...
  */
    ...
    */
    async fetchAttribute(locator: string, attName: string) {
        ...
        if (!eleValue) throw new Error(`Element not found for locator: ${locator}`);
        return eleValue.evaluate((node, attributeName) => node.getAttribute(attributeName), attName);
    }
    ...
    */
    ...
    async verificationInputValue(locator: string, expectedTextSubstring: string) {
        const element = this.page.locator(locator).nth(0);
        await this.wait("minWait");
